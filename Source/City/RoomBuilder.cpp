	...
	for (FRoomPolygon *rp : roomPols) {
		for (int i = 1; i < rp->points.Num(); i++) {
			...
			if (rp->entrances.Contains(i)) {
				...
				if (floor == 0) {
					FPolygon currWindow;
					FVector pw1 = rp->points[i - 1] + tangent * 50 + FVector(0, 0, 50 + windowHeight);
					FVector pw2 = pw1 - FVector(0, 0, windowHeight);
					FVector pw3 = rp->points[i] - tangent * 50 + FVector(0, 0, 50);
		...
		for (int j = 0; j < testsPerSide; j++) {
			...
			if (onWall) {
				...
				for (int k = 1; k < pol.points.Num(); k++) {
					...
					if (FVector::DistSquared(curr, pol.points[k]) < 1500) { // pick a number that seems reasonable
						count++;
					}
				}
				if (count < 2) {
...
static TArray<FMeshInfo> getBathRoom(FRoomPolygon *r2, TMap<FString, UHierarchicalInstancedStaticMeshComponent*> &map) {
	...
	attemptPlace(r2, placed, meshes, false, 5, "toilet" , FRotator(0, 270, 0), FVector(0, 0, 0), map, false);
	FTransform res = attemptGetPosition(r2, placed, meshes, false, 5, "sink", FRotator(0, 0, 0), FVector(0, 0, 0), map, false);
...
static TArray<FMeshInfo> getHallWay(FRoomPolygon *r2, TMap<FString, UHierarchicalInstancedStaticMeshComponent*> &map) {
	...
	attemptPlace(r2, placed, meshes, true, 1, "hanger", FRotator(0, 0, 0), FVector(0, 0, 20), map, false);
	return meshes;
}
...
static TArray<FMeshInfo> getKitchen(FRoomPolygon *r2, TMap<FString, UHierarchicalInstancedStaticMeshComponent*> &map) {
	...
	attemptPlace(r2, placed, meshes, false, 4, "kitchen", FRotator(0, 90, 0), FVector(-15, 0, 0), map, true);
	...
	attemptPlace(r2, placed, meshes, false, 3, "oven", FRotator(0, 270, 0), FVector(0, 0, 0), map, true);
	return meshes;
}
...
static TArray<FMeshInfo> getCorridor(FRoomPolygon *r2, TMap<FString, UHierarchicalInstancedStaticMeshComponent*> &map) {
	...
	placed.Append(getBlockingVolumes(r2, 200, 100));
	attemptPlace(r2, placed, meshes, false, 5, "wardrobe", FRotator(0, 0, 0), FVector(0, 0, 0), map, true);
